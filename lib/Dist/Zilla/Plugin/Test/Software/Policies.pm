    ...
    default => sub {
        ...
        if( @{ $self->include_policy() } ) {
            ...
            foreach my $include_policy (@{ $self->include_policy() }) {
                $zilla->log_debug( [
                        'Include policy %s from include_policy config item',
                        $include_policy
                    ] );
        ...
        foreach my $policy (keys %check_policies) {
            ...
            $args{policy} = $policy;
            $self->zilla()->log_debug( [ 'Looking for matching policy: %s:%s:%s:%s',
...
sub mvp_multivalue_args { return qw(
...
sub _get_policy_config {
    ...
    my $plain_plugin = first {
        $_->isa('Dist::Zilla::Plugin::Software::Policies')
        && $_->plugin_name eq 'Software::Policies' } @{ $zilla->{'plugins'} };
    if( $plain_plugin ) {
